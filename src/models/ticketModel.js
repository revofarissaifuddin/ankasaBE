...
const createTicket = (data) => {
  ...
  return new Promise((resolve, reject) =>
    pool.query(
      `INSERT INTO tickets(id,airline_id,origin,destination,takeoff,landing,transit,facilites,duration,price,flight_class,terminal,gate,create_at) VALUES('${id}','${airline_id}','${origin}','${destination}','${takeoff}','${landing}','${transit}','${facilites}','${duration}','${price}','${flight_class}','${terminal}','${gate}','${create_at}')`,
      (err, result) => {
        if (!err) {
          resolve(result);
        } else {
