...
const createBooking = (data) => {
  const { id, users_id, tickets_id, passanger_id, title, is_paid, insurance } =
    data;
  let created_at = new Date().toISOString();
  return new Promise((resolve, reject) =>
    pool.query(
      `INSERT INTO bookings(id, users_id, tickets_id, passanger_id,title, is_paid, insurance, created_at) VALUES('${id}', '${users_id}', '${tickets_id}', '${passanger_id}','${title}', '${is_paid}', '${insurance}', '${created_at}')`,
      (err, result) => {
        if (!err) {
          resolve(result);
        } else {
